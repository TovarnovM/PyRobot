...
class Grid:
    def __init__(self, size_screen, size_node):
        ...
        self.matrix = numpy.zeros(1, dtype=numpy.int)
    ...
    def refresh(self, layer):
        ...
        self.flagstart = False
        self.flaggoal = False
        self.matrix = numpy.zeros((self.size[1] // self.size_node,
                                   self.size[0] // self.size_node), dtype=numpy.int)
    ...
    def drawrect(self, layer, color, pos):
        pygame.draw.rect(layer, color,
                         (pos[0] // self.size_node * self.size_node + 1,
                          ...
                          self.size_node - 1, self.size_node - 1))
    ...
    def drawpath(self, path, main_layer, flag):
        ...
        if flag == 1:
            ...
            time1 = nowayfont1.render('A*:' + str(path[1]), True, (255, 255, 255))
            main_layer.blit(time1, (16, 11))
            main_layer.blit(time, (15, 10))
        elif flag == 2:
            time = nowayfont.render('JPS:' + str(path[1]), True, (34, 177, 86))
            time1 = nowayfont1.render('JPS:' + str(path[1]), True, (255, 255, 255))
            main_layer.blit(time1, (6, 36))
            main_layer.blit(time, (5, 35))
        elif flag == 3:
            time = nowayfont.render('JPS_numba:' + str(path[1]), True, (34, 177, 86))
            time1 = nowayfont1.render('JPS_numba:' + str(path[1]), True, (255, 255, 255))
            main_layer.blit(time1, (6, 51))
            main_layer.blit(time, (5, 50))
        else:
            time = nowayfont.render('JPS_c:' + str(path[1]), True, (34, 177, 86))
            time1 = nowayfont1.render('JPS_c:' + str(path[1]), True, (255, 255, 255))
            main_layer.blit(time1, (6, 86))
            main_layer.blit(time, (5, 85))
        ...
        if flag == 1:
            ...
            while i < len(path[0]) - 1:
                pygame.draw.line(self.PathA, astarcolor,
                                 (path[0][i][1] * self.size_node + self.size_node // 2,
                                  path[0][i][0] * self.size_node + self.size_node // 2),
                                 (path[0][i + 1][1] * self.size_node + self.size_node // 2,
                                  path[0][i + 1][0] * self.size_node + self.size_node // 2),
                                 self.size_node // 2)
                i += 1
            ...
            i = 0
            while i < len(path[0]) - 1:
                pygame.draw.line(self.PathJ, jpscolor,
                                 (path[0][i][1] * self.size_node + self.size_node // 2,
                                  path[0][i][0] * self.size_node + self.size_node // 2),
                                 (path[0][i + 1][1] * self.size_node + self.size_node // 2,
                                  path[0][i + 1][0] * self.size_node + self.size_node // 2),
    ...
    def mark_border(self, pos, main_layer):
        if ((pos[1] // self.size_node, pos[0] // self.size_node) != self.start and
                (pos[1] // self.size_node, pos[0] // self.size_node) != self.goal):
            if self.matrix[pos[1] // self.size_node][pos[0] // self.size_node] == 0:
                self.matrix[pos[1] // self.size_node][pos[0] // self.size_node] = 1
    ...
    def clear_border(self, pos, main_layer):
        if ((pos[1] // self.size_node, pos[0] // self.size_node) != self.start and
                (pos[1] // self.size_node, pos[0] // self.size_node) != self.goal):
            if self.matrix[pos[1] // self.size_node][pos[0] // self.size_node] == 1:
                self.matrix[pos[1] // self.size_node][pos[0] // self.size_node] = 0
